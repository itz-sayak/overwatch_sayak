...
from overwatch_sayak.annotators.base import ImageType
from overwatch_sayak.draw.color import Color, unify_to_bgr
from overwatch_sayak.draw.utils import calculate_optimal_text_scale, draw_text
from overwatch_sayak.geometry.core import Point
from overwatch_sayak.utils.conversion import (
    convert_for_image_processing,
    cv2_to_pillow,
    images_to_cv2,
)
from overwatch_sayak.utils.iterables import create_batches, fill
...
def crop_image(
    ...
    === "OpenCV"
        ...
        cropped_image = ov.crop_image(image=image, xyxy=xyxy)
        cropped_image.shape
        # (400, 400, 3)
        ```
    ...
    ![crop_image](https://media.roboflow.com/overwatch_sayak-docs/crop-image.png){ align=center width="800" }
    """  # noqa E501 // docs
...
def scale_image(image: ImageType, scale_factor: float) -> ImageType:
    ...
    === "OpenCV"
        ...
        scaled_image = ov.scale_image(image=image, scale_factor=0.5)
        scaled_image.shape
        # (540, 960, 3)
        ```
...
def resize_image(
    ...
    ![resize_image](https://media.roboflow.com/overwatch_sayak-docs/resize-image.png){ align=center width="800" }
    """  # noqa E501 // docs
...
def letterbox_image(
    ...
    === "OpenCV"
        ...
        letterboxed_image = ov.letterbox_image(image=image, resolution_wh=(1000, 1000))
    ...
    ![letterbox_image](https://media.roboflow.com/overwatch_sayak-docs/letterbox-image.png){ align=center width="800" }
    """  # noqa E501 // docs
...
def overlay_image(
    ...
    Examples:
        ...
        result_image = ov.overlay_image(image=image, overlay=overlay, anchor=(200, 400))
        ```

    ![overlay_image](https://media.roboflow.com/overwatch_sayak-docs/overlay-image.png){ align=center width="800" }
    """  # noqa E501 // docs
...
class ImageSink:
    def __init__(
        ...
        Examples:
            ...
            frames_generator = ov.get_video_frames_generator(<SOURCE_VIDEO_PATH>, stride=2)

            with ov.ImageSink(target_dir_path=<TARGET_CROPS_DIRECTORY>) as sink:
...
def create_tiles(
    ...
    * if square-grid placement causes last row to be empty - number of rows is trimmed
    ...
    Args:
        ...
        tile_padding_color (Union[Tuple[int, int, int], ov.Color]): Color to be used in
        ...
        tile_margin_color (Union[Tuple[int, int, int], ov.Color]): Color of tile margin.
            If tuple provided - should be BGR.
